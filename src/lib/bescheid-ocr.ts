...
//
// Migriert aus cloud/apps/arbeitslos-portal/src/lib/bescheid-ocr.ts.
// Wird von BescheidScanPage genutzt — bisher hatte das Standalone die Logik
// inline; ausgelagerte Lib macht Tests/Reuse einfacher.
 ...
 * Falls back to empty string when no API is configured (UI prompts manual input).
 */
 ...
 * Callback-basiert, damit die UI Per-Page-Status-Updates bekommt.
 */
...
export function validateFile(file: File): { valid: boolean; error?: string } {
  ...
  if (!allowedTypes.includes(file.type)) {
    return {
      valid: false,
      error: `Dateityp "${file.type}" wird nicht unterstuetzt. Erlaubt: JPG, PNG, WebP, HEIC, PDF`,
    }
  }
  ...
  if (file.size > maxSize) {
    return {
      valid: false,
      error: `Datei zu gross (${(file.size / 1024 / 1024).toFixed(1)} MB). Maximum: 20 MB`,
    }
  }
